...
Twitter.prototype.login = function(mount, success) {
	...
	return function handle(req, res, next) {
		...
		if ( twauth && twauth.user_id && twauth.access_token_secret ) {
			res.writeHead(302, {'Location': success + "?"+ querystring.stringify(twauth) || '/'});
			res.end();
			return;
		...
		// Returning from Twitter with oauth_token
			...
			function(error, access_token_key, access_token_secret, params) {
				...
				if ( error ) {
					...
					res.writeHead(302, {'Location': success + "?" + querystring.stringify({access_token_secret: access_token_secret, access_token_key: access_token_key, user_id: user_id, screen_name: screen_name}) || '/'});
					res.end();
					return;
				}
			});
